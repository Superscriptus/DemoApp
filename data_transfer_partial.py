"""
PARTIAL VERSION - BEFORE SIMULATION DATA WAS COMPLETED FOR ALL PARAMETER COMBINATIONS.

Script to transfer data.bak from a local copy of main SuperScript/simulation_io directory into a stripped
down version at ./data.bak which is used to drive the Streamlit app.

usage:  python data_transfer_partial.py <path_to_simulation_io>

Note: currently only model_vars files are retained (more granular agent, project and network data.bak are stripped out).
...
for pps in PPS:
    ...
    to_path = "data.bak/pps_%d_dwl_%.1f_budget_%d_sd_%.3f_train_%.1f" % (pps, 0.1, 1, 0.99, 0.1)
...
for pps in PPS:
    ...
    to_path = "data.bak/pps_%d_dwl_%.1f_budget_%d_sd_%.3f_train_%.1f" % (pps, dwl, 1, sd, 0.1)
    copy_data(from_path, to_path)
...
to_path = "data.bak/pps_%d_dwl_%.1f_budget_%d_sd_%.3f_train_%.1f" % (pps, dwl, 1, 0.99, 0.1)
copy_data(from_path, to_path)
...
to_path = "data.bak/pps_%d_dwl_%.1f_budget_%d_sd_%.3f_train_%.1f" % (pps, dwl, budget, 0.99, 0.1)
copy_data(from_path, to_path)
...
to_path = "data.bak/pps_%d_dwl_%.1f_budget_%d_sd_%.3f_train_%.1f" % (pps, dwl, budget, 0.99, train)
copy_data(from_path, to_path)
